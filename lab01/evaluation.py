...
def precision_recall(expected_results: List[bool], actual_results: List[bool]) -> (float, float):
    ...
    FN = 0
    FP = 0
    for result in range(len(expected_results)):
        if (expected_results[result] == True) and actual_results[result] == True:
            TP += 1
        elif (expected_results[result] == True) and actual_results[result] == False:
            FN += 1
        elif (expected_results[result] == False) and actual_results[result] == True:
            FP += 1
        #elif (expected_results[result] == False) and actual_results[result] == False:
        #    matrixInfo[1][1] += 1
    precision = TP/(TP+FP)
    recall = TP/(TP+FN)
...
def F1_score(expected_results: List[bool], actual_results: List[bool]) -> float:
    ...
    if (p == 0) and (r == 0):
        #print('returning none for the values above')
        return acc
        #return None
    return ((2*r*p)/(r+p))
